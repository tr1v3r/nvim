...
local setGeneralKeys = function()
	...
	map("<LEADER>/", ":set splitbelow<CR>:split<CR>:res -5<CR>:term<CR>i")
		:noremap()
		:silent()
		:desc("open terminal panel")
		:set()
	map("<C-`>", ":set splitbelow<CR>:split<CR>:res -5<CR>:term<CR>i")
		:noremap()
		:silent()
...
local setEditorPlugKeys = function()
	...
	map("gbc")
		...
		:set()
	map("gc", "<Plug>(comment_toggle_linewise)")
		:mode("n")
		:noremap()
		:silent()
		:desc("edit: Toggle comment for line with operator")
		:set()
	map("gb", "<Plug>(comment_toggle_blockwise)")
		:mode("n")
		:noremap()
		:silent()
		:desc("edit: Toggle comment for block with operator")
		:set()
	map("gc", "<Plug>(comment_toggle_linewise_visual)")
		:mode("x")
		:noremap()
		:silent()
		:desc("edit: Toggle comment for line with selection")
		:set()
	map("gb", "<Plug>(comment_toggle_blockwise_visual)")
		:mode("x")
		:noremap()
		:silent()
...
local setToolPlugKeys = function()
	...
	map("<LEADER>tr", "TroubleToggle lsp_references")
		:mode("n")
		:cr()
		:noremap()
		:silent()
	...
	map("<LEADER>td", "TroubleToggle document_diagnostics")
		...
		:set()
	map("<LEAEDR>tw", "TroubleToggle workspace_diagnostics")
		:mode("n")
		:cr()
		:noremap()
		:silent()
	...
	map("<LEADER>fc", "Telescope colorscheme")
		:mode("n")
		:pure()
		:noremap()
		:silent()
	...
	map("<LEADER>fz", "Telescope zoxide list")
		:mode("n")
		:pure()
		:noremap()
		:silent()
